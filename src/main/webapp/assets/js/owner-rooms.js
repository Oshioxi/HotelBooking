...
(function() {
    ...
    async function loadRooms() {
        try {
            ...
            const tbody = document.querySelector('#roomsTable tbody');
            if (tbody) {
                tbody.innerHTML = '<tr><td colspan="9" class="text-center text-danger">Error loading rooms: ' + (error.message || 'Unknown error') + '</td></tr>';
            }
    ...
    function displayRooms(rooms) {
        ...
        if (!Array.isArray(rooms) || rooms.length === 0) {
            tbody.innerHTML = '<tr><td colspan="9" class="text-center text-muted">No rooms found. <button class="theme-btn mt-2" onclick="showAddRoomModal()">Add Your First Room</button></td></tr>';
            return;
        }
