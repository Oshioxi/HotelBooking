// Admin Hotels Management JavaScript
// Helper functions
...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        loadSidebarMenu(userRole, '/admin/hotels');
...
async function loadHotels() {
    try {
        ...
        // Use endpoint with owner info
        allHotels = await HotelBookingAPI.AdminAPI.getAllHotelsWithOwner();
        console.log('Hotels loaded:', allHotels);
        
        // Debug: Check images for each hotel
        allHotels.forEach(hotel => {
            console.log(`Hotel ${hotel.id} (${hotel.name}):`, {
                images: hotel.images,
                imagesCount: hotel.images ? hotel.images.length : 0
            });
        });
        ...
        const tbody = document.querySelector('#hotelsTable tbody');
        if (tbody) {
            tbody.innerHTML = '<tr><td colspan="11" class="text-center text-danger">Error loading hotels: ' + errorMsg + '</td></tr>';
        }
...
function filterHotels() {
    try {
        ...
        const filtered = status ? allHotels.filter(h => h && h.status === status) : allHotels;
...
function displayHotels(hotels) {
    ...
    if (!Array.isArray(hotels) || hotels.length === 0) {
        tbody.innerHTML = '<tr><td colspan="11" class="text-center text-muted">No hotels found</td></tr>';
        return;
    }
    ...
    hotels.forEach(hotel => {
            ...
            if (primaryImage && primaryImage.imageUrl) {
                ...
                if (imageUrl) {
                    ...
                    imagesHtml += `<img src="${imageUrl}" 
                        ...
                        onclick="showImageGallery(${hotel.id})">`;
        ...
        const ownerName = hotel.ownerName || (hotel.owner && hotel.owner.fullName) || ('Owner ID: ' + (hotel.ownerId || 'N/A'));
        ...
        html += '<tr>' +
            ...
            '<td style="white-space: nowrap;">' +
                '<button class="btn btn-sm btn-info me-1" onclick="manageHotelDetails(' + hotel.id + ')" title="Manage Images & Amenities">' +
                    '<i class="far fa-cog"></i>' +
                '</button>' +
                (hotel.status === 'PENDING' ? 
                    '<button class="btn btn-sm btn-success me-1" onclick="approveHotel(' + hotel.id + ')" title="Approve">' +
                        '<i class="far fa-check"></i> Approve' +
                    '</button>' +
                    '<button class="btn btn-sm btn-danger me-1" onclick="rejectHotel(' + hotel.id + ')" title="Reject">' +
                        '<i class="far fa-times"></i> Reject' +
                    '</button>' 
                : '') +
                '<button class="btn btn-sm btn-danger" onclick="deleteHotel(' + hotel.id + ')" title="Delete">' +
...
async function approveHotel(id) {
    ...
    if (!confirm('Approve this hotel? It will be visible to users.')) {
        return;
    }
    
    try {
        await HotelBookingAPI.AdminAPI.approveHotel(id);
        alert('Hotel approved successfully');
        await loadHotels();
    } catch (error) {
        console.error('Error approving hotel:', error);
        alert('Error approving hotel: ' + (error.message || 'Unknown error'));
    }
}
...
async function rejectHotel(id) {
    ...
    try {
        await HotelBookingAPI.AdminAPI.rejectHotel(id, reason);
        alert('Hotel rejected successfully');
        await loadHotels();
    } catch (error) {
        console.error('Error rejecting hotel:', error);
        alert('Error rejecting hotel: ' + (error.message || 'Unknown error'));
    }
}
...
async function deleteHotel(id) {
    ...
    try {
        await HotelBookingAPI.AdminAPI.deleteHotel(id);
...
async function manageHotelDetails(hotelId) {
    if (!hotelId) {
        alert('Invalid hotel ID');
        return;
    }
    ...
    try {
        ...
        const hotel = allHotels.find(h => h && h.id === hotelId);
...
async function loadHotelImages(hotelId) {
    ...
    try {
        ...
        const images = await HotelBookingAPI.HotelImageAPI.getByHotel(hotelId);
...
function displayImages(images) {
    ...
    images.forEach(image => {
        ...
        html += '<div class="col-md-3 mb-3">' +
            ...
            '<button class="btn btn-sm btn-danger mt-2" onclick="deleteImage(' + imageId + ')">' +
...
async function loadHotelAmenities(hotelId) {
    ...
    try {
        ...
        const hotelAmenities = await HotelBookingAPI.HotelAmenityAPI.getByHotel(hotelId);
...
function displayAmenities(amenities) {
    ...
    amenities.forEach(amenity => {
        ...
        html += '<div class="col-md-4 mb-2">' +
            ...
            '</div>' +
            '<button class="btn btn-sm btn-danger" onclick="removeAmenity(' + hotelAmenityId + ')">' +
...
async function saveImage() {
    try {
        ...
        // Upload image
        const uploadResult = await HotelBookingAPI.FileUploadAPI.uploadHotelImage(file, currentHotelId);
        ...
        await HotelBookingAPI.HotelImageAPI.add(currentHotelId, imageData);
        ...
        await Promise.all([
            loadHotelImages(currentHotelId),
            loadHotels()
        ]);
...
async function deleteImage(imageId) {
    ...
    try {
        await HotelBookingAPI.HotelImageAPI.delete(currentHotelId, imageId);
...
async function showAddAmenityForm() {
    try {
        if (!currentHotelId) {
            alert('Error: No hotel selected');
            return;
        ...
        try {
            hotelAmenities = await HotelBookingAPI.AmenityAPI.getByCategory('HOTEL');
            ...
            try {
                ...
                try {
                    const allAmenities = await HotelBookingAPI.AmenityAPI.getAll();
        ...
        const hotel = allHotels.find(h => h && h.id === currentHotelId);
...
async function saveAmenity() {
    try {
        ...
        await HotelBookingAPI.HotelAmenityAPI.add(currentHotelId, amenityIdNum);
        ...
        await Promise.all([
            loadHotels(),
            loadHotelAmenities(currentHotelId)
        ]);
...
async function removeAmenity(hotelAmenityId) {
    if (!hotelAmenityId) {
        alert('Invalid amenity ID');
        return;
    }
    ...
    try {
        await HotelBookingAPI.HotelAmenityAPI.remove(currentHotelId, hotelAmenityId);
...
function showImageGallery(hotelId) {
    try {
        const hotel = allHotels.find(h => h && h.id === hotelId);
